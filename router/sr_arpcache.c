...
void sr_arpcache_handle_arpreq(struct sr_instance *sr, struct sr_arpreq *req)
{
    ...
    */
    ...
    struct sr_arpentry *arpentry;             /* ARP table entry */
    struct sr_arpreq *arpreq;                 /* request entry */
    ...
    if (difftime(curtime, req->sent) > 0.5)
    {
        ...
        if (req->times_sent >= 5)
        {
            ...
            fprintf(stderr, " didn't respond to 5 ARP requests. All queued packets will be dropped!\n");
            for (pck = req->packets; pck != NULL; pck = pck->next)
            {
                i_hdr0 = (struct sr_ip_hdr *)(((uint8_t *)(pck->buf)) + sizeof(struct sr_ethernet_hdr));
                ...
                ict3_hdr->icmp_code = 0x01; /* code : 1 */
                ...
                i_hdr->ip_src = ifc-> ip; /* i_hdr0->ip_dst; */
                ...
                /* refer ARP table */
                arpentry = sr_arpcache_lookup(cache, rtentry->gw.s_addr);
                if (arpentry != NULL)
                {
                    /* set dst MAC addr */
                    ...
                    fprintf(stderr, "Sending ICMP Host Unreachable to ");
                    print_addr_ip_int(ntohl(i_hdr->ip_dst));
                    fprintf(stderr, "... ");
        ...
        else
        {
            ...
            /* ARP header */
            a_hdr = (struct sr_arp_hdr *)(((uint8_t *)e_hdr) + sizeof(struct sr_ethernet_hdr)); /* a_hdr set */
            ...
            print_addr_ip_int(ntohl(a_hdr->ar_tip));
            fprintf(stderr, "... ");
